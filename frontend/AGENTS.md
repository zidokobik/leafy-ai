...
- `src/components/` holds the shell (`AppShell`, `AppSidebar`, `Topbar`) and shared UI. `src/components/ui/` contains shadcn/ui source components generated by the shadcn CLI. `navigation.ts` is the single source of truth for the sidebar entries and page titles.
...
- `src/styles/base.css` holds Tailwind imports, shadcn theme tokens, global resets, and the basil-green visual theme. Do not recreate broad custom layout stylesheets for shell/page UI that should be composed from shadcn primitives.
- `src/assets/` stores images imported by TypeScript modules.
- `components.json` configures shadcn/ui aliases, Tailwind integration, icon library and style preset.
...
Use shadcn/ui primitives from `src/components/ui/` for reusable interface structure: `Sidebar` for app navigation, `Card` for grouped content, `Field` with `Input` for forms, `Dialog` for account settings, `ToggleGroup` for small option sets, `Alert` for feedback, `Empty` for placeholder states, and `Chart` wrappers for Recharts. Add or update shadcn components with `npx shadcn@latest ...` from the `frontend/` directory so `components.json` and aliases are respected.

Prefer semantic Tailwind tokens such as `bg-background`, `text-muted-foreground`, `border-border`, `bg-primary`, and `text-sidebar-foreground`. Keep theme changes in `src/styles/base.css`; avoid raw color utility classes in feature components unless a domain visualization requires a specific chart color.
